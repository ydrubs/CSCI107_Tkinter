...
def pick_sequence():
    ...
    while True:
        ...
        if len(game_sequence) == 0:
            ...
            break
        #Make sure the current value is not the same as last value in the sequence
...
def play_sequence():
    ...
    #Without delay button, all buttons will anitmate at the same time. The dealy adds the 'time' variable to each.after()
    delay = 0
...
def check_round():
    ...
    #Player is occrect, so change the label, update score, wait, then start next round
    if player_sequence == game_sequence:
        change_label('Correct!')
        score += len(player_sequence) + int(1000/time)
        root.after(500, pick_sequence)
    #The player is incorrect so change the label, update score, disable buttons, and reset for new game
    else:
        change_label('Wrong!')
        score = 0
        disable()
        #Game is over so wipe game sequence
        game_sequence = []
...
def disable():
    '''Disable all buttons so they can't accidentally be pressed'''
